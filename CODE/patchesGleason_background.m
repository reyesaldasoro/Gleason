...
for currentFile= 1:15: numFiles
   ...
   % currentImageNamePathS           = strcat(saveDir,currentImageName(1:end-3),'png');
    ...
    [backgroundMask] = detectBackground(currentImageR);
        ...
        for k1=1:256:rows-256
            ...
            for k2=1:256:cols-256
                ...
                if (sum(sum(1-backgroundMask(rr,cc)))/256/256)>0.99
                    currPatch=currentImageR(rr,cc,:);
                    currentImageNamePathS  = strcat(saveDirP,'B_',currentImageName(1:end-4),num2str(k1),'_',num2str(k2),'.png');
                    imwrite(currPatch,currentImageNamePathS)
                    currentImageNamePathL  = strcat(saveDirL,'B_',currentImageName(1:end-4),num2str(k1),'_',num2str(k2),'_L.png');
                    imwrite(basicPatch*multiplierPatch,currentImageNamePathL);
